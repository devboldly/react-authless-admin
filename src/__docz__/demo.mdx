...
However, those direct links are rather convenient for admins, especially if you'd have to do some searching otherwise. And since this uses local storage, the admin state persists between browser sessions as well. 🙌
...
import { useAdmin, AdminOnly, NonAdminOnly, NetlifyCMSButton, AdminToggle } from 'react-authless-admin';
